 ...
 */
	...
	function EPANEL_WPStyleFont(element, property){
 ...
 */
	...
	function setLayoutBuilder(LayoutMode, margin, innereast, innerwest, gutter){
		...
		window['OuterLayout'] = jQuery("."+LayoutMode+" .layout-main-content").layout({ 
					...
					,	east__resizerClass: 	'epanel-resizer-east'
					,	west__resizerClass: 	'epanel-resizer-west'
					,	east__size:				margin
					,	west__size:				margin
					,	east__minSize:			10
					,	west__minSize:			10
					...
					, 	west__onresize: function (pane, $Pane, paneState) {
						...
						var position = jQuery("."+LayoutMode+" .epanel-resizer-west").position();
						jQuery("."+LayoutMode+" .epanel-resizer-east").css('right', position.left);
					...
					, 	east__onresize: function (pane, $Pane, paneState) {
						...
						var position = jQuery("."+LayoutMode+" .epanel-resizer-east").css('right');
						jQuery("."+LayoutMode+" .epanel-resizer-west").css('left', position);
						updateDimensions(LayoutMode);
					}
		});
	...
	jQuery('.image_upload_button').each(function(){
		...
		new AjaxUpload(clickedID, {
			  ...
			  data: { // Additional data to send
					action: 'epanel_ajax_post_action',
					type: 'upload',
					data: clickedID },
			  ...
			  onComplete: function(file, response) {
				...
				if(response.search('Upload Error') > -1){
					...
					var buildReturn = '<img style="max-width:'+previewSize+'px;" class="epanel_image_preview" id="image_'+clickedID+'" src="'+response+'" alt="" />';
...
function EPANEL_WPSimpleToggle(showElement, hideElement){
...
function EPANEL_WPSlideToggle(toggle_element, toggle_input, text_element, show_text, hide_text, option){
	var opt_value; 
	var input_flag;
	...
	var data = {
		action: 'epanel_ajax_save_option',
		option_name: option,
