...
/*-----------------------------------------------------------------------------------*/
/* Options Framework metabox options handler
/*-----------------------------------------------------------------------------------*/

class EPANEL_WPMetaPanel {
	...
	function __construct( $settings = array() ) {
		...
		$defaults = array(
				...
				'id' 		=> 'epanel-metapanel',
				'name' 		=> 'Additional Pages or Posts Settings',
	...
	function add_metapanel_box(){
		...
		foreach($this->settings['posttype'] as $post_type){
			add_meta_box($this->settings['id'], $this->settings['name'], "epanel_metapanel_callback", $post_type, $this->settings['location'], $this->settings['priority'], array( $this ));
		}
	...
	function draw_meta_options(){ global $post_ID;  ?>
		...
		<?php if(!$this->hide_tabs):
			...
			if(isset($_COOKIE['EPANEL_WPMetaTabCookie']))
				$selected_tab = (int) $_COOKIE['EPANEL_WPMetaTabCookie'];
			else
				$selected_tab = 0;
		?>
			<script type="text/javascript">
					...
					jQuery(document).ready(function($) {						
						...
						$('#metatabs').bind('tabsshow', function(event, ui) {
							...
							$.cookie('EPANEL_WPMetaTabCookie', selectedTab);
						});
		...
		<?php endif;?>
			...
			<div id="metatabs" class="epanel_metapanel fix">
				<?php if(!$this->hide_tabs):?>
					<ul id="metatabsnav" class="epanel_metapanel_tabs">
				...
				<?php endif;?>
				<div class="epanel_metapanel_pad fix <?php if($this->hide_tabs) echo 'hide_tabs';?>">
					<div class="metapanel_panes fix">
						
						<div class="epanel_metapanel_options">
						
							<div class="epanel_metapanel_options_pad">
								<?php foreach($this->tabs as $tab => $t):?>
									<div id="<?php echo $tab;?>" class="epanel_metatab">
					...
					<div class="epanel_metapanel_footer fix" style="display:none;">
							<input type="hidden" name="_posttype" value="<?php echo $this->settings['posttype'];?>" />
							<input id="update" class="button-primary" type="submit" value="<?php _e("Save Meta Settings",'epanel'); ?>" accesskey="p" tabindex="5" name="update"/>
					</div>
				</div>
	...
	function meta_option_engine($oid, $o){
		...
		// Check if correct post type
			...
			// Test Version
			if(NOT_SET || (!NOT_SET && $o['version'] != 'pro')){ ?>
	...
	function option_breaker($oid, $o){
		...
		switch ( $o['type'] ){
			...
			case 'image_upload' :
				...
				break;
			default :
				do_action( 'epanel_metapanel_' . $o['type'] , $oid, $o);
				break;
	...
	function _get_check_option($oid, $o){ global $post_ID;?>
			...
			<input class="admin_checkbox" type="checkbox" id="<?php echo $oid;?>" name="<?php echo $oid;?>" <?php checked((bool) m_epanel($oid, $post_ID)); ?> />
<?php }
	
	function _get_text_option($oid, $o){ global $post_ID;?>
			<?php if(isset($o['label'])):?><label class="metatext-label"><?php echo $o['label'];?></label><?php endif;?>
			<input type="text" class="html-text <?php echo 'metatext-'.$o['size'];?>"  id="<?php echo $oid;?>" name="<?php echo $oid;?>" value="<?php em_epanel($oid, $post_ID); ?>" />
<?php }

	function _get_textarea_option($oid, $o){ global $post_ID;?>
			<?php if(isset($o['label'])):?><label class="metatext-label"><?php echo $o['label'];?></label><?php endif;?>
			<textarea class="html-textarea <?php echo 'metatextarea-'.$o['size'];?>"  id="<?php echo $oid;?>" name="<?php echo $oid;?>" /><?php em_epanel($oid, $post_ID); ?></textarea>
	<?php }
			...
			<select id="<?php echo $oid;?>" name="<?php echo $oid;?>">
				...
				<?php foreach($o['selectvalues'] as $sval => $sset):?>
					<?php if($o['type']=='select_same'):?>
							<option value="<?php echo $sset;?>" <?php if(get_epanel_meta($oid, $post_ID)==$sset) echo 'selected';?>><?php echo $sset;?></option>
					<?php elseif(is_array($sset)):
						$disabled_option = (isset($sset['version']) && $sset['version'] == 'pro' && !NOT_SET) ? true : false;
					?>
						<option <?php if($disabled_option) echo 'disabled="disabled" class="disabled_option"';?> value="<?php echo $sval;?>" <?php if(get_epanel_meta($oid, $post_ID)==$sval) echo 'selected';?>><?php echo $sset['name']; if($disabled_option) echo ' (pro)';?></option>
					<?php else:?>
							<option value="<?php echo $sval;?>" <?php if(get_epanel_meta($oid, $post_ID)==$sval) echo 'selected';?>><?php echo $sset;?></option>
					<?php endif;?>
	...
	function _get_select_count_option($oid, $o){  global $post_ID;?>
		...
		<select id="<?php echo $oid;?>" name="<?php echo $oid;?>">
			...
			<?php for($i = $count_start; $i <= $o['count_number']; $i++):?>
					<option value="<?php echo $i;?>" <?php selected($i, get_epanel_meta($oid, $post_ID)); ?>><?php echo $i;?></option>
			<?php endfor;?>
		</select>
	<?php }
		...
		<?php if(is_array($terms_array) && !empty($terms_array)):?>
				...
				<select id="<?php echo $oid;?>" name="<?php echo $oid;?>">
					...
					<?php foreach($terms_array as $term):?>
						<option value="<?php echo $term->slug;?>" <?php if(get_epanel_meta($oid, $post_ID)==$term->slug) echo 'selected';?>><?php echo $term->name; ?></option>
					<?php endforeach;?>
				</select>
		...
		<select id="<?php echo $oid;?>" name="<?php echo $oid;?>">
			...
			<?php	$menus = wp_get_nav_menus( array('orderby' => 'name') );
					foreach ( $menus as $menu )
						printf( '<option value="%d" %s>%s</option>', $menu->term_id, selected($menu->term_id, get_epanel_meta($oid, $post_ID)), esc_html( $menu->name ) );
			?>
		</select>
...
function epanel_metapanel_callback($post, $object){
...
function do_global_meta_options(){
	...
	$metatab_settings = array(
			...
			'icon' =>  CORE_IMAGES.'/admin/ileaf.png'
		);
