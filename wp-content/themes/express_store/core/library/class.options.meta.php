...
/*-----------------------------------------------------------------------------------*/
/* Options Framework metabox handler
/* i'm tired - no comments for this file....
/*-----------------------------------------------------------------------------------*/

class EPANEL_WPMetaOptions {
	...
	function __construct($meta_array, $settings = array()) {
		...
		$defaults = array(
				...
				'id' => 'EPANEL_WP-Meta-Options',
				'name' => 'EPANEL_WP Custom Options',
				'posttype' => null,
				'location' => 'normal', 
				'priority' => 'low'
			);
	...
	function add_meta_options_box(){
		...
		if(is_array($this->settings['posttype'])){
			foreach($this->settings['posttype'] as $post_type){
				add_meta_box($this->settings['id'], $this->settings['name'], "epanel_meta_options_callback", $post_type, $this->settings['location'], $this->settings['priority'], array( $this ));
			}
		}else{
			add_meta_box($this->settings['id'], $this->settings['name'], "epanel_meta_options_callback", $this->settings['posttype'], $this->settings['location'], $this->settings['priority'], array( $this ));
		}
	...
	<?php foreach($this->meta_array as $optionid => $o): ?>
						...
						<div class="page_option">
								...
								<?php if($o['type']=='check'):?>
									...
									<div class="option-inputs">
											...
											<input class="admin_checkbox" type="checkbox" id="<?php echo $optionid;?>" name="<?php echo $optionid;?>" <?php checked((bool) m_epanel($optionid, $post_ID)); ?> />
						
									</div>
								<?php elseif($o['type'] == 'textarea'):?>
									
										<div class="option-inputs">
											<textarea class="html-textarea"  id="<?php echo $optionid;?>" name="<?php echo $optionid;?>" /><?php em_epanel($optionid, $post_ID); ?></textarea>
										</div>
								
								<?php elseif($o['type'] == 'text' || $o['type'] == 'text_small' || $o['type'] == 'text_link' || $o['type'] == 'text_big'):?>

										<div class="option-inputs">
											<input type="text" class="html-text <?php echo 'meta-'.$o['type'];?>"  id="<?php echo $optionid;?>" name="<?php echo $optionid;?>" value="<?php em_epanel($optionid, $post_ID); ?>" />
										</div>
												...
												<select id="<?php echo $optionid;?>" name="<?php echo $optionid;?>">
													...
													<?php foreach($o['selectvalues'] as $sval => $sset):?>
														<?php if($o['type']=='select_same'):?>
																<option value="<?php echo $sset;?>" <?php if(get_epanel_meta($optionid, $post_ID)==$sset) echo 'selected';?>><?php echo $sset;?></option>
														<?php elseif(is_array($sset)):
															$disabled_option = (isset($sset['version']) && $sset['version'] == 'pro' && !VPRO) ? true : false;
														?>
															<option <?php if($disabled_option) echo 'disabled="disabled" class="disabled_option"';?> value="<?php echo $sval;?>" <?php if(get_epanel_meta($optionid, $post_ID)==$sval) echo 'selected';?>><?php echo $sset['name']; if($disabled_option) echo ' (pro)';?></option>
														<?php else:?>
																<option value="<?php echo $sval;?>" <?php if(get_epanel_meta($optionid, $post_ID)==$sval) echo 'selected';?>><?php echo $sset;?></option>
														<?php endif;?>
									...
									<div class="option-inputs">
										...
										<select id="<?php echo $optionid;?>" name="<?php echo $optionid;?>">
											...
											<?php for($i = $count_start; $i <= $o['count_number']; $i++):?>
													<option value="<?php echo $i;?>" <?php selected($i, get_epanel_meta($optionid, $post_ID)); ?>><?php echo $i;?></option>
											<?php endfor;?>
										</select>
									</div>
								...
								<?php elseif($o['type'] == 'image_upload'):?>
									...
									<div class="option-inputs">
										<p>
											...
											<input class="regular-text uploaded_url" type="text" name="<?php echo $optionid;?>" value="<?php em_epanel($optionid, $post_ID); ?>" /><br/><br/>
										...
										<?php if(m_epanel($optionid, $post_ID)):?>
											<img class="epanel_image_preview" id="image_<?php echo $optionid;?>" src="<?php em_epanel($optionid, $post_ID); ?>" style="max-width: 100px"/>
										<?php endif;?>
									</div>
									...
									<?php if(is_array($terms_array) && !empty($terms_array)):?>
											...
											<select id="<?php echo $optionid;?>" name="<?php echo $optionid;?>">
												...
												<?php foreach($terms_array as $term):?>
													<option value="<?php echo $term->slug;?>" <?php if(get_epanel_meta($optionid, $post_ID)==$term->slug) echo 'selected';?>><?php echo $term->name; ?></option>
												<?php endforeach;?>
											</select>
			...
			<div class="page_option fix update-meta-options">
					...
					<input id="update" class="button-primary" type="submit" value="<?php _e("Update Options",'epanel'); ?>" accesskey="p" tabindex="5" name="update"/>
			</div>
		</div>
...
function epanel_meta_options_callback($post, $object){
