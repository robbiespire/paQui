 ...
 * FSStorage
 *
 * Note that storage modules cannot use EcartError for logging errors
...
class FSStorage extends StorageModule implements StorageEngine {
	...
	function actions () {
		add_action('wp_ajax_ecart_storage_suggestions',array(&$this,'suggestions'));
 		add_filter('ecart_verify_stored_file',array(&$this,'verify'));
	}
	...
	function save ($asset,$data,$type='binary') {
		...
		if ($type == "upload") { // $data is an uploaded temp file path, just move the file
			...
			if (!is_readable($data)) die("$this->module: Could not read the file."); // Die because we can't use EcartError
	...
	function settings () {
		...
		$default = __('The file system path to your storage directory.','Ecart');
		...
		foreach ($this->settings['path'] as $method => &$path) {
			...
			elseif	(!file_exists($p)) $error = __("The path does not exist.","Ecart");
			elseif	(!is_dir($p)) $error = __("The path supplied is not a directory.","Ecart");
			elseif	(!is_writable($p)) $error = __("The path must be <strong>writable</strong> by the web server.","Ecart");
			elseif	(!is_readable($p)) $error = __("The path must be <strong>readable</strong> by the web server.","Ecart");
	...
	function suggestions () {
		...
		check_admin_referer('wp_ajax_ecart_storage_suggestions');
		if (empty($_GET['q']) || strlen($_GET['q']) < 3) return;
		if ($_GET['t'] == "image") $this->context('image');
		else $this->context('download');
		
		global $Ecart;
		if ($Ecart->Storage->engines[$this->context] != $this->module) return;
