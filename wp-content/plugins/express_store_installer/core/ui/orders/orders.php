<div class="wrap ecart">
	...
	<h2><?php _e('Shopping Cart Orders History','Ecart'); ?></h2>
	...
	<p id="post-search" class="search-box" style="display:none !important;">
		...
		<input type="submit" value="<?php _e('Search Orders','Ecart'); ?>" class="button" />
	</p>

	<?php if (current_user_can('ecart_financials')): ?>
	<ul id="report" style="display:none !important;">
		<li><strong><?php echo $ordercount->total; ?></strong> <span><?php _e('Orders','Ecart'); ?></span></li>
		<li><strong><?php echo money($ordercount->sales); ?></strong> <span><?php _e('Total Sales','Ecart'); ?></span></li>
		<li><strong><?php echo money($ordercount->avgsale); ?></strong> <span><?php _e('Average Sale','Ecart'); ?></span></li>
	</ul>
	<?php endif; ?>

	<div class="tablenav">
		<div class="alignleft actions inline">
		<?php if (current_user_can('ecart_delete_orders')): ?><button type="submit" id="delete-button" name="deleting" value="order" class="button-secondary"><?php _e('Delete','Ecart'); ?></button><?php endif; ?>
			<select name="newstatus">
				<?php echo menuoptions($statusLabels,false,true); ?>
			</select>
			<button type="submit" id="update-button" name="update" value="order" class="button-secondary"><?php _e('Update','Ecart'); ?></button>
			<div class="filtering" style="display:none !important;">
				...
				<button type="submit" id="filter-button" name="filter" value="order" class="button-secondary"><?php _e('Filter','Ecart'); ?></button>
			</div>
			</div>
	...
	<table class="widefat" cellspacing="0">
		<thead>
		<tr><?php print_column_headers('toplevel_page_ecart-orders'); ?></tr>
		</thead>
		<tfoot>
		<tr><?php print_column_headers('toplevel_page_ecart-orders',false); ?></tr>
		</tfoot>
	<?php if (sizeof($Orders) > 0): ?>
		<tbody id="orders-table" class="list orders">
		<?php
			$hidden = get_hidden_columns('toplevel_page_ecart-orders');
			...
			do_action_ref_array('ecart_order_row_css',array(&$classes,&$Order));
			$even = !$even;
			?>
		<tr class="<?php echo join(' ',$classes); ?>">
			...
			<td class="name column-name"><a class='row-title' href='<?php echo esc_url(add_query_arg(array('page'=>'ecart-orders','id'=>$Order->id),admin_url('admin.php'))); ?>' title='<?php _e('View','Ecart'); ?> &quot;<?php echo $Order->id; ?>&quot;'><?php echo esc_html(empty($Order->firstname) && empty($Order->lastname))?"(".__('no contact name','Ecart').")":"{$Order->firstname} {$Order->lastname}"; ?></a><?php echo !empty($Order->company)?"<br />".esc_html($Order->company):""; ?></td>
			<td class="destination column-destination<?php echo in_array('destination',$hidden)?' hidden':''; ?>"><?php
				...
				if (isset($Order->downloads)) echo (!empty($location)?'<br />':'').__('Downloads','Ecart');
				?></td>
	...
	<?php else: ?>
		<tbody><tr><td colspan="6"><?php _e('No','Ecart'); ?><?php if (!empty($_GET['status'])) echo ' '.strtolower($statusLabels[$_GET['status']]); ?> <?php _e('orders, yet.','Ecart'); ?></td></tr></tbody>
	<?php endif; ?>
	</table>
	...
	<div class="tablenav" style="display:none !important;">
		<?php if (current_user_can('ecart_financials') && current_user_can('ecart_export_orders')): ?>
		<div class="alignleft actions">
			...
			<div id="export-columns" class="multiple-select">
				<ul>
					...
					<li<?php if ($even) echo ' class="odd"'; $even = !$even; ?>><input type="hidden" name="settings[purchaselog_headers]" value="off" /><input type="checkbox" name="settings[purchaselog_headers]" id="purchaselog_headers" value="on" /><label for="purchaselog_headers"><strong><?php _e('Include column headings','Ecart'); ?></strong></label></li>
...
var lastexport = new Date(<?php echo date("Y,(n-1),j",$Ecart->Settings->get('purchaselog_lastexport')); ?>);

jQuery(document).ready( function() {
var $=jqnc();

pagenow = 'toplevel_page_ecart-orders';
...
$('#delete-button').click(function() {
	if (confirm("<?php echo addslashes(__('Are you sure you want to delete the selected orders?','Ecart')); ?>")) return true;
	else return false;
});

$('#update-button').click(function() {
	if (confirm("<?php echo addslashes(__('Are you sure you want to update the status of the selected orders?','Ecart')); ?>")) return true;
	else return false;
});
